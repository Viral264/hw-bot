...
def init_db():
    ...
    """)
    # На случай, если база уже существовала без новых колонок — добавим их
    for col, coltype in (("file_id", "TEXT"), ("file_type", "TEXT")):
        try:
...
def add_homework(user_id: int, subject: str, description: str, deadline: str,
                  file_id: str | None = None, file_type: str | None = None):
    conn = sqlite3.connect(DB_PATH)
    cur = conn.cursor()
    cur.execute(
        "INSERT INTO homework (user_id, subject, description, deadline, file_id, file_type, created_at) "
        "VALUES (?, ?, ?, ?, ?, ?, ?)",
        (user_id, subject, description, deadline, file_id, file_type, datetime.now().isoformat()),
    )
    conn.commit()
    conn.close()


def get_homework(user_id: int, only_pending=True, days_ahead: int | None = None):
    conn = sqlite3.connect(DB_PATH)
    cur = conn.cursor()
    query = "SELECT id, subject, description, deadline, done, file_id, file_type FROM homework WHERE user_id = ?"
    params = [user_id]
    if only_pending:
        query += " AND done = 0"
...
def get_one(user_id: int, hw_id: int):
    conn = sqlite3.connect(DB_PATH)
    cur = conn.cursor()
    cur.execute(
        "SELECT id, subject, description, deadline, done, file_id, file_type "
        "FROM homework WHERE id = ? AND user_id = ?",
        (hw_id, user_id),
    )
    row = cur.fetchone()
    conn.close()
...
def mark_done(user_id: int, hw_id: int) -> bool:
    conn = sqlite3.connect(DB_PATH)
    cur = conn.cursor()
    cur.execute("UPDATE homework SET done = 1 WHERE id = ? AND user_id = ?", (hw_id, user_id))
...
def delete_homework(user_id: int, hw_id: int) -> bool:
    conn = sqlite3.connect(DB_PATH)
    cur = conn.cursor()
...
def get_due_tomorrow_unreminded():
    ...
    cur.execute(
        "SELECT id, user_id, subject, description FROM homework "
...
def hw_actions_kb(hw_id: int, has_file: bool) -> InlineKeyboardMarkup:
    """Кнопки под конкретным заданием: готово / удалить / показать файл."""
    buttons = [
        [
...
def format_hw_line(hw_id, subject, description, deadline, done, file_id=None, file_type=None) -> str:
...
async def cmd_start(message: Message):
    await message.answer(
        ...
        "К заданию можно прикрепить файл (фото или документ) — например, фото условия из тетради.",
...
async def finish_adding(message_or_cb, state: FSMContext, user_id: int,
                         file_id: str | None = None, file_type: str | None = None):
    data = await state.get_data()
    add_homework(user_id, data["subject"], data["description"], data["deadline"], file_id, file_type)
    await state.clear()
...
async def process_attachment_photo(message: Message, state: FSMContext):
    file_id = message.photo[-1].file_id  # берём самое большое разрешение
    text = await finish_adding(message, state, message.from_user.id, file_id, "photo")
...
async def process_attachment_document(message: Message, state: FSMContext):
    ...
    text = await finish_adding(message, state, message.from_user.id, file_id, "document")
    await message.answer(text, reply_markup=main_menu_kb())
...
async def process_attachment_skip(callback: CallbackQuery, state: FSMContext):
    text = await finish_adding(callback, state, callback.from_user.id)
...
async def send_hw_list(message: Message, rows, empty_text: str, header: str):
    ...
    for hw_id, subject, description, deadline, done, file_id, file_type in rows:
        text = format_hw_line(hw_id, subject, description, deadline, done, file_id, file_type)
...
async def cmd_list(message: Message):
    rows = get_homework(message.from_user.id)
    await send_hw_list(message, rows, "🎉 Нет невыполненных заданий!", "📋 Твои домашние задания:")


@router.message(Command("today"))
@router.message(F.text == "🔥 Сегодня")
async def cmd_today(message: Message):
    rows = get_homework(message.from_user.id, days_ahead=0)
...
async def cmd_week(message: Message):
    rows = get_homework(message.from_user.id, days_ahead=7)
    await send_hw_list(message, rows, "На этой неделе всё сдано или заданий нет 👍", "📆 На неделю:")


# ============ ГОТОВО / УДАЛИТЬ / ФАЙЛ — через команды ============
...
async def cmd_done(message: Message):
    ...
    if len(parts) != 2 or not parts[1].isdigit():
        ...
        return
    hw_id = int(parts[1])
    if mark_done(message.from_user.id, hw_id):
...
async def cmd_delete(message: Message):
    ...
    if len(parts) != 2 or not parts[1].isdigit():
        ...
        return
    hw_id = int(parts[1])
    if delete_homework(message.from_user.id, hw_id):
...
async def cb_done(callback: CallbackQuery):
    ...
    if mark_done(callback.from_user.id, hw_id):
        await callback.message.edit_text(f"✅ Задание #{hw_id} отмечено как выполненное!")
    else:
...
async def cb_delete(callback: CallbackQuery):
    hw_id = int(callback.data.split(":")[1])
    if delete_homework(callback.from_user.id, hw_id):
        await callback.message.edit_text(f"🗑️ Задание #{hw_id} удалено.")
    else:
        await callback.answer("Задание не найдено", show_alert=True)
        return
...
async def cb_file(callback: CallbackQuery):
    hw_id = int(callback.data.split(":")[1])
    row = get_one(callback.from_user.id, hw_id)
...
async def send_reminders():
    ...
    for hw_id, user_id, subject, description in rows:
        try:
